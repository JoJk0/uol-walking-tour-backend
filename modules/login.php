<!-- LOGIN PAGE -->
    ...
    <div id="login">
      ...
      <form action="?" method="POST" enctype="multipart/form-data">
        <input type="text" placeholder="<?php echo $_LANG['LOGIN_EMAIL']; ?>" name="email" required class="input"/>
        <input type="password" placeholder="<?php echo $_LANG['LOGIN_PASSWORD']; ?>" name="password" required class="input"/>
        <input type="submit" name="submit" value="<?php echo $_LANG['LOGIN_BUTTON']; ?>" class="button main centered">
      </form>
...
if(isset($_POST["submit"])){
  ...
  //~~~~~~~~~~~~~~~~~~~~~CHECKING EMAIL~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		$queryAdmin = "SELECT Username FROM Admin WHERE Username = '$post_login_email'"; 
		$resultAdmin = mysqli_query($con,$queryAdmin); 
		if(mysqli_num_rows($resultAdmin) > 0){


                $queryPassword = "SELECT * FROM Admin WHERE Username = '$post_login_email' ";
                                     ...
                                     if($row['deleted'] == 0){
                                              ...
                                              if($row['verified'] == 1){ // If user accepted by superuser
                                                ...
                                                if($row['AdminID'] == 1){
                                                  ...
                                                  $_SESSION['su'] = FALSE;//NO super user
                                                }
                                                ...
                                                throw_error($_LANG['LOGIN_USER_NOT_VERIFIED']);//LOGIN USER NOT VERIFIED
                                              }

                                     }else{
                                       throw_error($_LANG['LOGIN_USER_DELETED']);//LOGIN USER DELETED
                                     }  


                                  }else{
                                    throw_error($_LANG['LOGIN_INVALID_CREDENTIALS']);//INVALID CREDENTIALS
                                  }
                                }
